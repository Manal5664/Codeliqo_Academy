...
- Public marketing site with all six program pathways, curriculum, fees, admissions, FAQs, policies, contact information and public certificate verification.
- Student LMS for program and batch details, lessons, assignments, projects, progress, payments, certificates and profile updates.
- Admin LMS for secure student account onboarding, enrollments, programs, batches, lessons, assignments, projects, submission review, attendance, payments, certificates and announcements.
...
Only the Supabase project URL and **publishable key** belong in the browser. Never add a secret/service-role key, database password or other secret to a `VITE_` variable. The public site remains usable when Supabase is not configured; login and certificate lookup display a controlled unavailable state.

## Supabase setup

This workspace does not contain a project-specific `supabase/config.toml`. Initialize the CLI metadata once (this preserves the existing migrations/functions), authenticate, then link and apply migrations in order:
...
Files may be hosted in private Supabase Storage buckets or another trusted host; store only the resulting authorized URL in the relevant LMS record.
